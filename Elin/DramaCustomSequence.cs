...
public class DramaCustomSequence : EClass
{
	...
	public void Build(Chara c)
	{
			...
			if (!c.IsPCParty || EClass._zone.IsPCFaction)
			{
				...
				if (!EClass._zone.IsUserZone)
				{
					...
					if (c.trait.ShopType != ShopType.None)
					{
						...
						if (c.trait is TraitMerchantTravel && !c.IsPCFactionOrMinion && !c.isSummon)
						{
							Choice2("daRob", "_rob");
						}
					}
		...
		Method(delegate
		{
			...
			foreach (Chara item8 in EClass.pc.party.members.Where((Chara chara) => chara.CanBloom()))
			{
				Chara c2 = item8;

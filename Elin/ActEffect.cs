...
public class ActEffect : EClass
{
	...
	public static bool DamageEle(Card CC, EffectId id, int power, Element e, List<Point> points, ActRef actref, string lang = null, Point center = null)
	{
		...
		foreach (Point p in points)
		{
			...
			case EffectId.Missile:
			{
				...
				effect = Effect.Get((string)id2);
				if (id == EffectId.Arrow || id == EffectId.Missile)
				{
			...
			case EffectId.Earthquake:
			{
				...
				{
					pos2.Animate(AnimeID.Quake, animeBlock: true);
				});
				if (effect != null)
				{
			...
			default:
			{
				...
				if (id == EffectId.GravityGun)
				{
					...
					{
						pos.Animate(AnimeID.Gravity, animeBlock: true);
					});
				}
				break;
			}
			...
			if (effect2 != null)
			{
				effect2.SetParticleColor(elementRef.colorTrail, changeMaterial: true, "_TintColor").Play(point);
			}
			if (effect != null)
			{
				switch (id)
				...
				case EffectId.Missile:
					...
					{
						effect.Play(CC.pos);
						effect.GetComponent<MissileHomingEffect>().Launch(CC.pos.PositionCenter(), p.PositionCenter());
					});
					break;
				case EffectId.Arrow:
				case EffectId.MoonSpear:
			...
			foreach (Card item in p.ListCards().ToList())
			{
				...
				else
				{
					...
					if (num5 != 0)
					{
						num4 = num4 * 100 / (90 + center.Distance(p) * num5);
					}
				}
				...
				{
					int num6 = CC.Evalue(302);
					if (!CC.IsPC && CC.IsPCFactionOrMinion)
					{
						num6 += EClass.pc.Evalue(302);
					}
					...
					if (num6 > 0)
					{
						...
						num4 = EClass.rnd(num4 * 100 / (100 + num6 * 10 + 1));
						if (c == c.pos.FirstChara)
						{
				...
				if (c.IsAliveInCurrentZone)
				{
					...
					case EffectId.DrainMana:
						...
						{
							long num7 = num4 * num / 100;
							Debug.Log(num4 + " v:" + num7 + " evalue:" + e.Value + " power:" + power + " elepMod:" + num);
							if (num7 > c.Chara.mana.value)
							{
								num7 = c.Chara.mana.value;
							}
							c.Chara.mana.Mod((int)(-num7));
							CC.Chara.mana.Mod((int)num7);
						}
						break;
					}
				}
				...
				num2++;
				void AddCon<T>(int rate, int p2) where T : Condition
				{
					if (EClass.rnd(1 + actref.refVal) == 0 && !c.Chara.HasCondition<T>() && EClass.rnd(rate) == 0)
					{
						c.Chara.AddCondition<T>(p2);
					}
				}
			}
			if ((id == EffectId.Explosive || id == EffectId.Suicide || id == EffectId.Rocket) && (!EClass._zone.IsPCFaction || !EClass.Branch.HasItemProtection))
			{
				...
				{
				default:
					num8 = ((actref.refThing != null) ? actref.refThing.material.hardness : (30 + power / 20));
					break;
				case EffectId.Suicide:
					num8 = CC.LV / 3 + 40;
					break;
				...
				if (p.HasObj && p.cell.matObj.hardness <= num9)
				{
				...
				if (!p.HasObj && p.HasBlock && p.matBlock.hardness <= num9)
				{
	...
	public static void ProcAt(EffectId id, int power, BlessedState state, Card cc, Card tc, Point tp, bool isNeg, ActRef actRef = default(ActRef))
	{
		...
		case EffectId.Earthquake:
		{
			...
			if (CC.IsInMutterDistance() && !EClass.core.config.test.disableShake2)
			{
		...
		case EffectId.Meteor:
		{
			...
			List<Point> list10 = EClass._map.ListPointsInCircle(CC.pos, 10f);
			if (list10.Count == 0)
			{
				list10.Add(CC.pos.Copy());
			}
			...
			EClass.Wait(1f, CC);
			DamageEle(CC, id, power, element, list10, actRef, "spell_ball");
			return;
		}
		case EffectId.Comet:
		{
			...
			List<Point> list2 = EClass._map.ListPointsInCircle(tp, 7f);
			if (list2.Count == 0)
			{
				list2.Add(tp.Copy());
			}
			...
			EClass.Wait(1f, CC);
			DamageEle(CC, id, power, element, list2, actRef, "spell_ball", tp);
			return;
		}
		case EffectId.Hand:
		...
		case EffectId.Missile:
		{
			...
			List<Point> list8 = new List<Point> { tp.Copy() };
			foreach (Point item in EClass._map.ListPointsInCircle(tp, 6f, mustBeWalkable: false))
			{
				if (item.Equals(tp) || !Los.IsVisible(CC.pos, item))
				{
					continue;
				}
				foreach (Card item2 in item.ListCards())
				{
					if ((item2.isChara && CC.IsHostile(item2.Chara)) || item2.trait.CanBeAttacked)
					{
						list8.Add(item);
						break;
					}
			...
			CC.Say("spell_missile", CC, element.Name.ToLower());
			EClass.Wait(0.5f, CC);
			TryDelay(delegate
			{
			...
			for (int num9 = 0; num9 < 6 + EClass.rnd(7); num9++)
			{
				...
				DamageEle(CC, id, power, element, list7, actRef, "spell_missile");
			}
			return;
		}
		case EffectId.Summon:
		{
			...
			if (!(n == "special"))
			{
				if (n == "special2")
				{
					...
					{
						foreach (Chara item3 in EClass._map.charas.Where((Chara _c) => _c.id == "cocoon").ToList())
						{
							if (!item3.pos.IsSunLit)
							{
								item3.pos.PlayEffect("darkwomb3");
								item3.HatchEgg();
							}
						}
					}
			...
			for (int num6 = 0; num6 < num3; num6++)
			{
				...
				{
				case "special":
					if (num6 == 0 && !CC.HasMinion("imolonac") && !CC.HasMinion("ygolonac"))
					{
				...
				if (chara2.rarity >= Rarity.Legendary && !flag4)
				{
					num6--;
					continue;
				}
				...
				if (!(n == "shadow"))
				{
					...
					else
					{
						...
						if (!CC.IsPC)
						{
							num8 = (CC.IsPCFactionOrMinion ? (CC.LV / 2) : (CC.LV / 3 * 2));
						}
						if (num5 == -1)
						{
							num7 = chara2.LV * (100 + power / 10) / 100 + power / 30;
						}
						if (num7 < num8)
						{
							num7 = num8;
		...
		case EffectId.Bit:
		{
			...
			chara.SetLv(Mathf.Abs(power) / 15);
			chara.interest = 0;
		...
		case EffectId.Breathe:
		{
			...
			if (CC.IsInMutterDistance() && !EClass.core.config.graphic.disableShake && !EClass.core.config.test.disableShake2)
			{
		...
		case EffectId.GravityGun:
		{
			...
			List<Point> list9 = EClass._map.ListPointsInCircle((id == EffectId.GravityGun || id == EffectId.Rocket || id == EffectId.Flare) ? tp : cc.pos, radius2, !flag5, !flag5);
			if (list9.Count == 0)
			{
				list9.Add(cc.pos.Copy());
			}
			...
			if (id != EffectId.GravityGun && cc.IsInMutterDistance() && !EClass.core.config.graphic.disableShake && !EClass.core.config.test.disableShake2)
			{
				Shaker.ShakeCam("ball");
			}
			DamageEle(actRef.origin ?? cc, id, power, element, list9, actRef, (id == EffectId.Suicide) ? "suicide" : "spell_ball");
		...
		List<Card> list11 = tp.ListCards().ToList();
		list11.Reverse();
		if (list11.Contains(CC))
		{
			list11.Remove(CC);
			list11.Insert(0, CC);
		}
		bool flag6 = true;
		foreach (Card item5 in list11)
		{
			if (tc == null || item5 == tc)
			{
				Proc(id, power, state, CC, item5, actRef);
				if (flag2 && item5.isChara && item5 != CC)
				{
					CC.DoHostileAction(item5);
				}
	...
	public static void Proc(EffectId id, int power, BlessedState state, Card cc, Card tc = null, ActRef actRef = default(ActRef))
	{
		...
		case EffectId.Exterminate:
		{
			...
			Msg.Say("exterminate");
			List<Chara> list2 = EClass._map.charas.Where((Chara chara3) => (chara3.isCopy || chara3.isHatchling || chara3.id == "cocoon" || chara3.id == "cocoon_alien") && !chara3.IsPCFactionOrMinion).ToList();
			if (list2.Count == 0)
			{
		...
		{
			bool flag2 = id == EffectId.SilverCatMigration;
			if (!EClass.game.IsSurvival && ((flag2 && EClass._zone.id != "startVillage2") || (!flag2 && !(EClass._zone is Zone_LittleGarden))))
			{
				Msg.SayNothingHappen();
				return;
			}
			...
			{
				if (!chara3.IsPCFactionOrMinion && chara3.id == (flag2 ? "cat_silver" : "littleOne"))
				{
					if (flag3)
					{
						flag3 = false;
			...
			Msg.Say(flag2 ? "cat_migration" : "little_migration", list.Count.ToString() ?? "");
			EClass._zone.ModInfluence(list.Count);
			if (flag2)
			{
				EClass.player.stats.catDepart += list.Count;
				break;
			}
			EClass.player.stats.sistersDepart += list.Count;
		...
		case EffectId.ForgetItems:
		{
			...
			int num8 = power / 50 + 1 + EClass.rnd(3);
			List<Thing> source = TC.things.List((Thing thing8) => thing8.c_IDTState == 0);
		...
		case EffectId.EnchantArmorGreat:
		{
			...
			int num10 = (flag8 ? 4 : 2) + (blessed ? 1 : 0);
			if (tc.encLV >= num10)
			{
		...
		{
			bool flag7 = id == EffectId.GreaterIdentify;
			if (flag)
			{
				Redirect(EffectId.ForgetItems, flag7 ? BlessedState.Cursed : BlessedState.Normal, default(ActRef));
				break;
			}
			if (!tc.isThing || (actRef.refThing == null && cc.IsPC && tc.GetRootCard() == tc))
			{
				int count = ((!blessed) ? 1 : (flag7 ? (2 + EClass.rnd(2)) : (3 + EClass.rnd(3))));
				LayerDragGrid.CreateIdentify(tc.isChara ? tc.Chara : CC, flag7, state, 0, count);
				return;
			}
			cc.PlaySound("identify");
			cc.PlayEffect("identify");
			tc.Thing.Identify(cc.IsPCParty, (!flag7) ? IDTSource.Identify : IDTSource.SuperiorIdentify);
			break;
		}
		...
		case EffectId.Reconstruction:
		{
			...
			int num7 = Mathf.Max(tc.genLv, tc.LV, EClass.player.stats.deepest);
			CardBlueprint.Set(new CardBlueprint
			{
				blesstedState = state
			});
			Thing thing2 = ThingGen.Create(tc.id, -1, (int)((long)num7 * (long)power / 400));
			thing2.genLv = num7;
			thing2.idSkin = tc.idSkin;
			if (tc.c_uidAttune != 0)
			{
		...
		case EffectId.ChangeMaterialGreater:
		{
			...
			if (row == null)
			{
				bool num4 = id == EffectId.ChangeMaterialGreater;
				bool flag6 = id == EffectId.ChangeMaterialLesser;
				string text = tc.Thing.source.tierGroup;
				...
				num5 = Mathf.Clamp(num5 + EClass.rnd(2), 0, 4);
				if (EClass.rnd(10) == 0)
				{
					text = ((text == "metal") ? "leather" : "metal");
				}
				SourceMaterial.TierList tierList = (text.IsEmpty() ? tierMap.RandomItem() : tierMap[text]);
				for (int num6 = 0; num6 < 1000; num6++)
				{
					row = tierList.tiers[num5].Select();
					if (row != tc.material)
					{
						break;
					}
		...
		case EffectId.ChangeRarity:
		{
			...
			{
				int num3 = (tc.elements.Has(item4.id) ? tc.elements.GetElement(item4.id).vBase : 0);
				if (num3 != 0 && (Mathf.Abs(item4.vBase) <= Mathf.Abs(num3) || !MathEx.IsSameSign(item4.vBase, num3)))
				{
					continue;
				}
				int id2 = item4.id;
		...
		{
			Chara c = CC;
			if (!c.IsPC && !c.trait.UseRandomAlias && blessed)
			{
				c._alias = null;
				c.Say("reconstruct", c);
				SE.Change();
			...
			EClass.ui.AddLayer<LayerList>().SetStringList(delegate
			{
				...
				for (int i = 0; i < 10; i++)
				{
					list13.Add(AliasGen.GetRandomAlias());
				}
				return list13;
		...
		case EffectId.ChangeBiography:
			CC.bio.RerollParents();
			CC.Say("reconstruct", CC);
			SE.Change();
			CC.PlayEffect("mutation");
			break;
		case EffectId.ReturnVoid:
		{
			...
			int destLv = 1;
			Dialog.InputName("dialogVoidReturn".lang(max.ToString() ?? ""), max.ToString() ?? "", delegate(bool cancel, string str)
			{
				if (!cancel)
				{
					destLv = Mathf.Abs(str.ToInt());
					destLv = Mathf.Clamp(destLv, 1, max) * -1;
					Zone zone = ((root.lv == destLv) ? root : (root.children.Find((Spatial spatial) => spatial.lv == destLv) as Zone));
		...
		case EffectId.Gate:
			if (!tc.HasHost)
			{
				if (!flag)
				{
					if (id != EffectId.Gate && tc.isChara && tc.Chara.IsInCombat && tc.Chara.enemy != null && (tc.IsPCParty || id == EffectId.TeleportShort))
					{
						tc.Teleport(GetTeleportPos(tc.pos, 6, tc.Chara.enemy));
					}
					else if (id == EffectId.TeleportShort)
					{
			...
			if (tc.ExistsOnMap && tc.id == "mush_quantum" && !tc.HasElement(400) && !tc.HasCondition<ConGravity>())
			{
				if (EClass.rnd(2) == 0)
				{
					if (tc.IsGlobal || EClass._zone.IsInstance || tc.c_bossType != BossType.none)
					{
						tc.Die();
					}
					else
				...
				tc.Chara.TryDuplicate(DuplicateCondition.Quantum, null, inheritHostility: true);
			}
			if (flag)
			{
		...
		case EffectId.ThrowPotion:
			...
			{
				Thing t = ThingGen.Create(new string[6] { "330", "331", "334", "335", "336", "1142" }.RandomItem());
				ActThrow.Throw(CC, TC.pos, t, ThrowMethod.Punish, 0.7f);
			}
			break;
		...
		case EffectId.ShutterHex:
		{
			...
			TC.conditions.ForeachReverse(delegate(Condition condition4)
			{
				if (condition4.Type == ConditionType.Debuff && EClass.rnd(3) == 0)
				{
					condition4.Kill();
				}
			});
			...
			List<Chara> list10 = TC.pos.ListCharasInRadius(TC, 4, (Chara chara3) => chara3 == TC || chara3.IsHostile(CC));
			for (int num23 = 0; num23 < num22; num23++)
			{
				TweenUtil.Delay((float)num23 * 0.1f, delegate
				{
		...
		case EffectId.Steal:
		{
			...
			if (actRef.n1 == "money")
			{
				...
				if (currency > 0)
				{
					...
					thing7 = ThingGen.Create("money").SetNum(currency);
					TC.ModCurrency(-currency);
				}
			}
			else
			{
				Func<Thing, bool> func = (Thing thing8) => true;
				if (flag11)
				{
					func = (Thing thing8) => thing8.IsFood;
				}
				List<Thing> list12 = TC.things.List(delegate(Thing thing8)
				{
			...
			thing7.SetInt(116, 1);
			TC.PlaySound(thing7.material.GetSoundDrop(thing7.sourceCard));
			CC.Pick(thing7, msg: false);
			CC.Say("abSteal", CC, TC, thing7.Name);
			if (actRef.n1 == "food")
			{
				if (CC.hunger.value != 0)
				{
					CC.InstantEat(thing7);
				}
			}
			else
			{
		...
		case EffectId.CurseEQ:
		{
			...
			List<Thing> list4 = TC.things.List(delegate(Thing thing8)
			{
		...
		case EffectId.UncurseEQGreater:
		{
			...
			int success = 0;
			int fail = 0;
			List<Thing> list9 = new List<Thing>();
			TC.things.Foreach(delegate(Thing thing8)
			{
				...
				if ((thing8.isEquipped || thing8.IsRangedWeapon || blessed) && thing8.blessedState < BlessedState.Normal)
				{
					...
					if (power >= num26)
					{
						...
						if (thing8.isEquipped && thing8.HasElement(656))
						{
							TC.body.Unequip(thing8);
						}
		...
		case EffectId.Buff:
		{
			...
			Condition condition = Condition.Create(text2, power, delegate(Condition con)
			{
		...
		case EffectId.KizuamiTrick:
		{
			...
			int power2 = power;
			for (int num21 = 0; num21 < 4 + EClass.rnd(2); num21++)
			{
		...
		case EffectId.Wish:
			...
			Dialog.InputName("dialogWish", "q", delegate(bool cancel, string text4)
			{
				if (!cancel)
				{
					Msg.Say("wish", TC, text4);
					Wish(text4, EClass.pc.NameTitled, power, state);
				}
			});
			break;
		case EffectId.Faith:
		...
		case EffectId.TransBlood:
		{
			tc.PlaySound("mutation");
			tc.PlayEffect("mutation");
			int num18 = ((actRef.refThing != null) ? actRef.refThing.GetInt(118) : actRef.refVal);
			if (num18 == 0)
			{
				num18 = tc.GetInt(118);
				if (num18 == 0)
				{
					num18 = EClass.game.seed + tc.uid;
				}
				num18++;
			}
		...
		case EffectId.ResetFeat:
		{
			...
			{
				if (value.source.category != "feat" || value.id == 1644)
				{
					continue;
				}
				switch (value.source.categorySub)
				{
				case "general":
				case "attribute":
				case "skill":
			...
			foreach (Element item8 in list3)
			{
				...
				{
					for (int num12 = 0; num12 < item8.vBase - dNABonus; num12++)
					{
						num11 += item8.source.cost.TryGet(num12);
					}
					EClass.pc.SetFeat(item8.id, dNABonus);
				}
			}
			EClass.pc.feat += num11;
			EClass.pc.Refresh();
		...
		case EffectId.Weaken:
		{
			...
			int num19 = ((id == EffectId.DamageBody || id == EffectId.DamageMind) ? 1 : (4 + EClass.rnd(4)));
		...
		{
			bool flag12 = id == EffectId.RestoreBody;
			if (flag)
			{
				Redirect(flag12 ? EffectId.DamageBodyGreat : EffectId.DamageMindGreat, BlessedState.Normal, default(ActRef));
				break;
			}
			TC.Say(flag12 ? "restoreBody" : "restoreMind", TC);
			TC.PlaySound("heal");
			TC.PlayEffect("heal");
			TC.CureHost(flag12 ? CureType.CureBody : CureType.CureMind, power, state);
			if (blessed)
			{
				Redirect(flag12 ? EffectId.EnhanceBodyGreat : EffectId.EnhanceMindGreat, BlessedState.Normal, default(ActRef));
			}
			break;
		}
		...
		{
			long num13 = Dice.Create((actRef.act != null && EClass.sources.calc.map.ContainsKey(actRef.act.ID)) ? actRef.act.ID : "SpHealLight", power, CC, (actRef.refThing != null) ? null : actRef.act).Roll();
			if (actRef.refThing != null)
			{
				num13 = num13 * (100 + actRef.refThing.Evalue(750) * 10) / 100;
			}
			if (flag)
			{
				TC.DamageHP(num13 / 2, 919, power);
				break;
			}
			if (id == EffectId.JureHeal)
			{
				...
				TC.HealHP(num13, HealSource.Item);
				TC.Cure(CureType.Heal, power, state);
			}
			else
			{
				TC.HealHPHost(num13, (actRef.refThing == null) ? HealSource.Magic : HealSource.Item);
		...
		case EffectId.RemoveHexAll:
		{
			...
			IList<Condition> list5 = TC.conditions.Copy();
			list5.Shuffle();
			CC.PlaySound("remove_hex");
			CC.PlayEffect("remove_hex");
			foreach (Condition item9 in list5)
			{
				if (item9.Type != ConditionType.Debuff || item9.IsKilled)
				{
					continue;
				}
				if (EClass.rnd(power * (CC.IsPowerful ? 5 : 2)) > EClass.rnd(item9.power) || (id == EffectId.RemoveHex && EClass.rnd(2) == 0))
				{
					CC.Say("removeHex", TC, item9.Name.ToLower());
					item9.Kill();
				}
				else
				{
					...
					CC.Say("removeHex_fail", TC, item9.Name.ToLower());
				}
		...
		case EffectId.BlueDrag:
			...
			if (id == EffectId.BlueDrag)
			{
				...
				if (num24 > 0)
				{
					...
					if (num25 != 0)
					{
						TC.SetFeat(1423, num24 + num25, msg: true);
						TC.SetInt(134, TC.GetInt(134) + num25);
					}
				}
			}
			if (flag)
		...
		case EffectId.Acid:
		{
			...
			List<Thing> list8 = TC.things.List((Thing thing8) => (thing8.Num <= 1 && thing8.IsEquipmentOrRanged && !thing8.IsToolbelt && !thing8.IsLightsource && thing8.isEquipped) ? true : false);
			if (list8.Count != 0)
			{
		...
		int GetBlessingDifficulty(Condition condition4)
		{
			if (condition4.Type != ConditionType.Buff)
			{
				return 0;
			}
			if (condition4 is ConTransmuteBat && TC.HasCooldown(8793))
			{
				return 0;
			}
			if (!CC.IsHostile(TC))
			...
			if (condition4 is ConBoost)
			{
				return 5;
			}
			if (condition4 is ConRebirth)
			{
				return 10;
			}
			if (condition4 is ConInvulnerable)
			{
				return 100;
			}
			return 2;
	...
	public static Point GetShortTeleportPos(Point org, Point targetPos, int destDist, int maxOffset = 6)
	{
		for (int i = 0; i <= maxOffset; i++)
		{
			int destDist2 = destDist + i;
			List<Point> list = EClass._map.ListPointsOnEdge(targetPos, destDist2);
			list.RemoveAll((Point p) => !ValidPos(p));
			if (list.Count > 0)
			{
			...
			destDist2 = destDist - i;
			if (destDist2 >= 1)
			{
				list = EClass._map.ListPointsOnEdge(targetPos, destDist2);
				list.RemoveAll((Point p) => !ValidPos(p));
				if (list.Count > 0)
				{
					return list.RandomItem();
		...
		return GetTeleportPos(org);
		static bool ValidPos(Point p)
		{
			if (!p.IsValid || !p.IsInBounds || p.cell.blocked || p.cell.HasZoneStairs())
			{
				return false;
			}
			return true;
